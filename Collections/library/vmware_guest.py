...
class PyVmomiHelper(PyVmomi):
    ...
    def select_datastore(self, vm_obj=None):
            ...
            if self.params['disk'][0]['autoselect_datastore']:
                ...
                for ds in datastores:
                    ...
                    if (ds.summary.freeSpace > datastore_freespace) or (ds.summary.freeSpace == datastore_freespace and not datastore):
                        ...
                        datastore_name = datastore.name
                        datastore_freespace = ds.summary.freeSpace
                self.debugtest = str("entered first if")

            elif self.params['disk'][0]['datastore']:
                datastore_name = self.params['disk'][0]['datastore']

                try:
                    while self.params['disk'][self.ds_counter]['datastore']:
                        datastore_name = self.params['disk'][self.ds_counter]['datastore']
                        self.ds_name_list.append(datastore_name)
                        
                        datastore = self.cache.find_obj(self.content, [vim.Datastore], datastore_name)
                        self.ds_list.append(self.cache.find_obj(self.content, [vim.Datastore], datastore_name))
                        self.ds_counter += 1
                except:
                    pass
                ...
                #     self.ds_list.append(datastore)
                #     self.ds_counter += 1

                self.debugtest = str(self.ds_name_list)
            else:
        ...
        if not datastore and self.params['template']:
            ...
            self.debugtest = str("entered second if")
            self.store_vm_obj = vm_obj
            disks = [x for x in vm_obj.config.hardware.device if isinstance(x, vim.vm.device.VirtualDisk)]
            
            if disks:
                datastore = disks[0].backing.datastore
                self.ds_list.append(str(datastore))
                datastore_name = datastore.name
    ...
    def deploy_vm(self):
        ...
        if self.params['datastore']:
            ...
            # self.debugtest = str("exists in else")
            # self.ds_counter += 1
        ...
        try:
            if self.params['template']:
                ...
                disks = [x for x in self.store_vm_obj.config.hardware.device if isinstance(x, vim.vm.device.VirtualDisk)]
                ...
                if self.params['convert']:
                    for device in vm_obj.config.hardware.device:
                        if isinstance(device, vim.vm.device.VirtualDisk):
                            ...
                            disk_locator.datastore = disks[self.ds_counter].backing.datastore
                            self.relospec.disk.append(disk_locator)
                            # self.debugtest += str(device)

                # self.relospec.disk.append(disks)
                device = vim.vm.device.VirtualDisk()
                # self.debugtest = str(datastore)
                # self.ds_list.append(self.ds_counter)
...
def main():
    ...
    # VM doesn't exist
        ...
        if module.params['state'] in ['poweredon', 'powered-on', 'poweredoff', 'powered-off',
            ...
            result['debuginfo'] = pyv.debugtest
            result['debug_list'] = pyv.ds_list
            if result['failed']:
